...
@section('main')
    <div class="row">
        <div class="col-6">
            <h1 class="h3 mb-3 text-gray-800">Уровни критичности</h1>
        </div>
        <div class="col-6">
            <a href="{{ route('importance.create') }}" class="btn btn-primary float-right">Создать</a>
        </div>
    </div>
    ...
    <script>
        ...
        function ajaxRequest(params) {
            var url = '{{ route('importance.response') }}'
        ...
        function actionFormatter(value, row) {
            return '<div class="text-center"><a class="btn btn-primary btn-sm mr-1" href="/document-flow/importance/show/' + row.id + '">Просмотр</a>' +
                '<a class="btn btn-primary btn-sm" href="/document-flow/importance/edit/' + row.id + '">Править</a></div>';
        }
    </script>
